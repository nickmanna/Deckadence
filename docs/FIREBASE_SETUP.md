...
`src/firebase.js` reads its configuration from environment variables — the
config values are never hardcoded or committed to source control.

1. Copy the example env file: `cp .env.example .env`
2. Fill in `.env` with the values from your Firebase config object:
...
`.env` is gitignored — restart `npm start` after editing it, since
Create React App only reads these variables at process startup.
...
1. **The web `apiKey` is not a secret** — it identifies your project and is
   always visible in the compiled JS bundle. Keep it out of source control
   anyway (use `.env`, see above) so rotating it doesn't require a code
   change, and so it isn't tied to git history.
2. **Restrict the API key** in Google Cloud Console (APIs & Services >
   Credentials) to your app's HTTP referrers.
3. **Never put an Admin SDK / service account private key in client code**
   — that key *is* a secret and belongs only on a trusted server.
4. **Use Firestore/Storage security rules** as the real access-control layer
   — the client config alone grants no permissions.
5. **Enable Firebase App Check** in production to block traffic that isn't
   from your real app.
6. **Use HTTPS in production** (Firebase requires this)
7. **Regularly review Firebase security rules**
